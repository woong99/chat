          ...
          publicChatRooms.map((room) => (
            ...
            >
              <Card.Body className="position-relative">
                {room.name}
                {subscriptions.findIndex((item) => item.roomId === room.roomId) !== -1 && (
                  <Badge bg="primary" className="position-absolute end-0 me-4">
                    {subscriptions.find((item) => item.roomId === room.roomId).notRead === 0
                      ? '구독중'
                      : subscriptions.find((item) => item.roomId === room.roomId).notRead}
                  </Badge>
                )}
              </Card.Body>
            </Card>
